...
```{r classification_hyperparameter_tuning, eval = FALSE}

# Specify model ----------------------------------------------------------------
rf_class_tune_model <- parsnip::rand_forest(trees = 500,
                                      mtry = tune::tune(),
                                      min_n = tune::tune()) %>%
  # Specify the engine
...
# setup new workflow
rf_class_tune_wflow <- workflows::workflow() %>%
  workflows::add_recipe(rec) %>%
  workflows::add_model(rf_class_tune_model)

# define cross validation procedure
cv_folds <- rsample::vfold_cv(df_training_cat, v = 5)

#rf_class_grid <- grid_random(parameters(rf_class_tune_model), size = 100)
#rf_class_grid <- grid_regular(parameters(rf_class_tune_model), c(5,5))
rf_class_grid <- dials::grid_regular(dials::mtry(range = c(2, 20)),
...
# 1 random grid
set.seed(345)
rf_class_tuning <- tune::tune_grid(
  rf_class_tune_wflow,
  resamples = cv_folds,
  grid = rf_class_grid
)
...
# get metrics
metrics <- rf_class_tuning %>% tune::collect_metrics()
...
```{r classification_bestfit, eval = FALSE}
# Select and retrain best model ------------------------------------------------

best_auc <- tune::select_best(rf_class_tuning, "roc_auc")
...
rf_class_final_wflow <- workflows::workflow() %>%
  workflows::add_recipe(rec) %>%
  workflows::add_model(final_rf)

# train and test final model
rf_class_final_fit <- rf_class_final_wflow %>%
  tune::last_fit(data_split_cat)

# collect metrics
rf_class_final_fit %>%  tune::collect_metrics()

# get predictions
df_pred <- rf_class_final_fit %>% tune::collect_predictions()
...
yardstick::roc_curve(df_pred, truth = Qs_rel, estimate = .pred_low) %>%
  ggplot2::autoplot() +
...
```{r regression_bestfit, eval = FALSE}
